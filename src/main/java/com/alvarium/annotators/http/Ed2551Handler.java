...
public class Ed2551Handler implements Handler {

	HttpUriRequest request;

	public Ed2551Handler(HttpUriRequest request) {
	...
	public void addSignatureHeaders(Date ticks, String[] fields, SignatureInfo keys)
		...
		if (fields.length == 0) {
			throw new HandlerException("No fields found to be used in generating the seed");
		}
		...
		try {
			...
			throw new HandlerException("Invalid request URI", e);

		} catch (ParseResultException e1) {
			throw new HandlerException("Error parsing the request", e1);
		}
		...
		try {
			...
			throw new HandlerException("Invalid key type", e);
		}
		...
		try {
			...
			throw new HandlerException("Cannot read key.", e);
		}
		...
		try {
			...
			throw new HandlerException("Cannot sign annotation.", e);
		}
